                    ...
                    <form method="POST" action="{{ route('hearing-votes.update', $hearingVote) }}">
                        ...
                        <div class="mb-6">
                            ...
                            @else
                                            ...
                                            @foreach ($councillors as $councillor)
                                                <label class="flex items-center">
                                                    <input type="checkbox" 
                                                           name="votes_for[]" 
                                                           value="{{ $councillor->id }}"
                                                           {{ in_array($councillor->id, old('votes_for', $votesFor)) ? 'checked' : '' }}
                                            ...
                                            @foreach ($councillors as $councillor)
                                                <label class="flex items-center">
                                                    <input type="checkbox" 
                                                           name="votes_against[]" 
                                                           value="{{ $councillor->id }}"
                                                           {{ in_array($councillor->id, old('votes_against', $votesAgainst)) ? 'checked' : '' }}
                                ...
                                <p class="mt-2 text-xs text-gray-500">Select which councillors voted for or against this hearing. Councillors shown were serving in {{ $hearingVote->hearing->region->name }} at the time of this hearing.</p>
                            @endif
                        </div>
